...
def replay(fn: Callable):
    ...
    r = redis.Redis()
    func_nam = fn.__qualname__
    c = r.get(func_nam)
    try:
        c = int(c.decode("utf-8"))
    except Exception:
        c = 0
    print("{} was called {} times:".format(func_nam, c))
    inputs = r.lrange("{}:inputs".format(func_nam), 0, -1)
    outputs = r.lrange("{}:outputs".format(func_nam), 0, -1)
    for inp, outp in zip(inputs, outputs):
        ...
        try:
            ...
            outp = ""
        print("{}(*{}) -> {}".format(func_nam, inp, outp))
